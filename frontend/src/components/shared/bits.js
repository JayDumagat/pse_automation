...
export const RunStatusPill = ({ status, testId }) => {
  const map = {
    running: {
      label: "Running",
      cls: "bg-sky-400/15 text-sky-300 border-sky-400/30",
    },
    ready: {
      label: "Ready",
  ...
  return (
    <Badge
      data-testid={testId}
      variant="outline"
      className={`gap-1.5 font-medium ${m.cls}`}
    >
      <span
        className={`h-1.5 w-1.5 rounded-full ${status === "running" ? "bg-sky-400 stage-dot-running" : status === "failed" ? "bg-rose-400" : status?.startsWith("ready") ? "bg-emerald-400" : "bg-muted-foreground"}`}
      />
      {m.label}
    </Badge>
  );
...
export const SectionTitle = ({ children, className = "" }) => (
  <div
    className={`text-xs font-semibold tracking-widest uppercase text-muted-foreground ${className}`}
  >
    {children}
  </div>
);
